...
# 定义需要过滤的关键词
FILTER_KEYWORDS = ['拍1', '拍2', '拍3', '拍4', '拍5', '[聊天记录]','JORDAN','专卖店','淘宝','.tb.cn','.Tb.cn','taobao.com']
...
class AsyncCapture:
    def __init__(self, capturer):
        ...
        self.queue = Queue(maxsize=1)  # 队列大小为1
        self.running = True
                                ...
                                for url in extracted_urls:
                                    ...
                                    if url:
                                        try:
                                            ...
                                            if result.stdout:
                                                print(f"\n[{timestamp}] [{duration:.1f}s]---------- Node.js 输出开始 ----------")
                                                #send_message_to_feishu(f"---------- 检测成功 ----------\n[{timestamp}] [{duration:.1f}s]")
                    
                                                modified_output = result.stdout.replace(
                                                    "领取成功！感谢您的参与，祝您购物愉快~", 
                                                    "领取成功"
                                                )
                                                message_with_time = f"{timestamp}运行{duration:.1f} 秒\n {modified_output}"
                                                send_message_to_feishu(message_with_time)
                                                print(f"{message_with_time}")
                                                #exclude_keywords = ["不存在直接访问", "已经领取过了", "已过期"]
                                                #if not any(keyword in modified_output for keyword in exclude_keywords):
                                                #    message_with_time = f"{timestamp}运行{duration:.1f} 秒\n {modified_output}"
                                                #    send_message_to_feishu(message_with_time)
                                                #else:
                                                #    # 打印具体排除原因
                                                #    excluded = [kw for kw in exclude_keywords if kw in result.stdout]
                                                #    print(f"检测到关键词：{', '.join(excluded)}，不发送消息到飞书")

                                            if result.stderr:
                                                print(f"\n[{timestamp}] ---------- Node.js 错误开始 ----------")
                                                print(result.stderr)
