...
class YOLOv5Detector:
    ...
    def detect(self, frame, img_size=None, conf_thres=0.5, iou_thres=0.45):
        if img_size is None:
        ...
        try:
            ...
            # ---------- GPU预处理 ----------
            # 将numpy数组转为torch张量（BGR顺序），并直接放到GPU上（保持uint8）
            tensor = torch.from_numpy(frame).to(self.device, non_blocking=True)  # shape: H, W, C
            # 调整维度顺序：HWC -> CHW
            tensor = tensor.permute(2, 0, 1).unsqueeze(0)  # shape: 1, C, H, W
            # BGR -> RGB（交换通道）
            tensor = tensor[:, [2, 1, 0], :, :]
            # 转换为float并归一化到[0,1]
            tensor = tensor.float() / 255.0
            # 使用GPU进行resize（双线性插值）
            tensor = torch.nn.functional.interpolate(tensor, size=(img_size, img_size), 
            ...
            # 后处理（NMS）仍在CPU
            return non_max_suppression(pred[0], conf_thres=conf_thres, iou_thres=iou_thres, max_det=10)
        except Exception as e:
            print(f"检测过程出错: {e}")
            return []
...
def main():
    ...
    # 配置参数
    cvshow = False               # 是否显示窗口（调试时可改为 True  False ）
    target_fps = 60               # 不再强制限制，仅用于参考
    ...
    try:
        # 初始化检测器（此时 detector 才存在）
        print("加载YOLOv5模型...")
        detector = YOLOv5Detector(model_path, 'cuda:0' if torch.cuda.is_available() else 'cpu')
        
        # 设置检测参数（可根据需要调整）
        detector.img_size = 640   # 保持原始分辨率（不降低精度）
        conf_thres = 0.6           # 原始置信度阈值
        iou_thres = 0.5            # 原始 IoU 阈值
            ...
            try:
                ...
                if filtered_targets:
                    for target in filtered_targets:
                        target_id = (target['class'], target['position'])
                        if target['class'] == 'connection' and click_controller.can_click(target_id):
                            ...
                            click_controller.mark_as_clicked(target_id)
                            
                            # 点击复制按钮（坐标偏移根据实际调整）
                            x2, y2 = x + 20, y + 12
                            time.sleep(0.1)
                            click_controller.perform_click(x2, y2, region, right_click=False)
                            time.sleep(0.1)
                ...
                # 显示窗口（仅当 cvshow=True 时）
                if cvshow:
                    fps_display = 1.0 / (time.time() - prev_time) if 'prev_time' in locals() else 0
                    prev_time = time.time()
        ...
        if 'async_capturer' in locals():
            async_capturer.stop()
        if 'capturer' in locals():          # 新增：显式关闭屏幕捕获器
            capturer.close()
